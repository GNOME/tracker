...
class DBusDaemon:
    ...
    def start_if_needed(self, config_file=None, env=None):
        if self.existing_session:
            ...
            self._threads=[threading.Thread(target=self.pipe_to_log, args=(self.process.stdout, dbus_stdout_log, self._stop_event), daemon=True),
                           threading.Thread(target=self.pipe_to_log, args=(self.process.stderr, dbus_stdout_log, self._stop_event), daemon=True)]
    ...
    def pipe_to_log(self, pipe, dbuslog, stop_event):
